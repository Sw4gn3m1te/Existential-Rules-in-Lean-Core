...
structure PseudoCoreChaseBranch (kb : KnowledgeBase sig) where
  ...
  core_node_eq :
    let last_node : ChaseNode obs kb.rules := (branch.get? last_index).get (terminates_at (last_index + 1) (Nat.lt_add_one last_index)).left
      ...
      have : ∀ (m : Nat), ∀ cn, cn ∈ branch.get? m → cn.facts.finite := by
        ...
        induction m generalizing cn with
          ...
          | succ m ih =>
            ...
            have : cn.facts = cm.facts ∪ trg.val.mapped_head[↑i].toSet := by grind
            rw [this]
...
namespace PseudoCoreChaseBranch
  ...
  instance : Membership (ChaseNode obs kb.rules) (PseudoCoreChaseBranch kb) where
  ...
  def build_pseudo_core_chase_branch_from_terminating_chase_branch (scb : ChaseBranch obs kb) (last_index : Nat)
    ...
    {
      branch := scb.branch
      database_first := scb.database_first
      triggers_active := scb.triggers_active
      triggers_exist := scb.triggers_exist
